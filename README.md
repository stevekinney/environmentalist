...
Recognized keys: `description`, `example`, `docs`, `secret`, plus two per-field source overrides. (`deprecated` is accepted by the registry's type but nothing reads it yet.) `env` forces a specific environment-variable name in the `env` source—_replacing_ the derived casing and any `envPrefix`, so the derived spelling is no longer consulted for that field—which is what lets you bind to a variable you don't control without a generic name like `FILE` hijacking it. (It scopes the process environment only; `.env` files are still read by their derived names.) `flag` does the same for a flag name.
